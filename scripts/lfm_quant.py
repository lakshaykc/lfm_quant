...
import os
import sys
from runtime.model_execution import ModelExecution
from runtime.cdrs_data import GenerateCDRSData

import configs

_dq_env_path = os.environ.get('LFM_QUANT_ROOT')
assert _dq_env_path is not None, "Environment Variable LFM_QUANT_ROOT not set"
...
def get_configs():
    ...
    configs.DEFINE_string("cdrs_inference_date", None, "CDRS Inference date. Format: '%Y-%m-%d' ")
    ...
    c.piecewise_lr_boundaries = [float(x) for x in c.piecewise_lr_boundaries.split('-')]
    c.piecewise_lr_values = [float(x) for x in c.piecewise_lr_values.split('-')]
...
def main():
    ...
    # if cdrs_ml_fname does not exist, it is created internally
    if config.cdrs_inference and not os.path.isfile(os.path.join(config.data_dir, config.cdrs_ml_fname)):
        GenerateCDRSData(config)()
        config.datafile = config.cdrs_ml_fname

    elif config.cdrs_inference and os.path.isfile(os.path.join(config.data_dir, config.cdrs_ml_fname)):
        config.datafile = config.cdrs_ml_fname

    else:
        pass

    model_exec = ModelExecution(config)()
